var mainPlatform = {
	...
	bindEvent: function(){
        ...
        $(document).on('click', '.sider-nav-s > li', function(e){
        	...
        	if($('.easyui-tabs1[arrindex=0]').tabs('exists', titleName)){
        		$('.easyui-tabs1[arrindex=0]').tabs('select', titleName)
        		return;
        	}
        	
        	//$.messager.progress({text:"加载中，请稍侯......"});
        	layer.load(0,{shade: [0.3, '#393D49'] });//0.1透明度的白色背景
        	$('.easyui-tabs1[arrindex=0]').tabs('add',{
				title: titleName,
        ...
        for(var i = 0, len = menu.menu.length; i < len; i++){
        	...
        	if(m.isCurrent){
        		if(m.children && m.children.length > 0) {
        			...
        			str = '<li class="current no-child" data-href="'+ m.href + "/" + m.id +'">';
        		}
        	}else{
        		if(m.children && m.children.length > 0) {
        			str = '<li>';
        		}else{
        			str = '<li class="no-child" data-href="'+ m.href + "/" + m.id +'">';
        		}
        	}
           ...
           str += '<a href="javascript:;" class="pf-menu-title">'+
            ...
            for(var j = 0, jlen = m.children.length; j < jlen; j++){
            	...
            	var dataHref = (child.children && child.children.length > 0) ? '' : ('data-href="' + child.href + "?menuId=" + child.id + '"'); 
            	if(child.isCurrent){
	...
	_renderThreeLevelMenu: function(list) {
		...
		for(;i<len;i++) {
			...
			html += '<li text="' + one.title +'" data-href="' + one.href + "?menuId=" + one.id + '"><a href="#">' + one.title + '</a></li>';
	...
	_createPageContainer: function(index){
	    ...
	    $('.easyui-tabs1[arrindex=0]').tabs('add',{
			title: "首页",
			content: '<iframe class="page-iframe" src="'+ webFirstUrl + '" frameborder="no" border="no" height="100%" width="100%" scrolling="auto"></iframe>',
			closable: false
		});
	}
